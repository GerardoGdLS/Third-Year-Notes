...
void IOCheck() {
    ...
    if (timer1_flag) {
        ...
        if (systemOn) {
            ...
            if(currentADCValue <= 0x0009){
                currentADCValue = 0;
            }
            currentBrightness = ((uint32_t)currentADCValue * MAX_PWM) / 0x03FF;
            PWM_SetDuty(ledOn ? currentBrightness : MIN_PWM);
        } else {
            PWM_SetDuty(ledOn ? MAX_PWM : MIN_PWM);
        }
...
void TransmitDataToUART() {
    ...
    if (systemOn) {
        ...
        // OFF mode - intensity is always 0
        intensity = 0;
        currentADCValue = 0;
    }
